# xarm-aio Single Arm Guide

本文档是 `xarm-aio` 的右臂单臂推理和硬件联调指南。它面向当前工程：

```text
openpi serve_policy.py(GPU/工作站)
        |
        | websocket
        v
xarm-aio/inference/infer_sync.py(机器人主机)
        |
        | xarm_can + ROS2 cameras
        v
xArm 右臂
```
...
### websocket `InvalidMessage: did not receive a valid HTTP response`

通常是代理影响了本机或局域网 websocket 连接。先确认服务端：
