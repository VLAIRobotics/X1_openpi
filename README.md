...
| `<D405_LEFT_WRIST_SERIAL>` | 左腕 RealSense 序列号 |
| `<D405_RIGHT_WRIST_SERIAL>` | 右腕 RealSense 序列号 |
| `<LEFT_CAN>` | 左臂 CAN 接口，默认示例为 `can0` |
| `<RIGHT_CAN>` | 右臂 CAN 接口，默认示例为 `can1` |

机器人与数据约定：

- 机械臂：X1 双臂，左臂默认 CAN 接口为 `can0`，右臂默认 CAN 接口为 `can1`。
...
pi05_x1_fold_towel:
  robot_mode: dual
  language_instruction: "Fold the towel twice"
  image_keys: ["cam_high", "cam_left_wrist", "cam_right_wrist"]
  left0: [-0.14133669435977936, 0.444007009267807, -0.05349050089716911,
...
`language_instruction` 必须与训练配置中的 `default_prompt` 相同；`left0` 和 `right0` 是该任务的双臂 home 位。上面的 `left0` 只是模板占位，上真机前必须替换成现场确认过的左臂安全 home 位。
...
TrainConfig(
    ...
    data=LeRobotAlohaDataConfig(
        repo_id="x1_fold_towel",
        assets=AssetsConfig(asset_id="x1_fold_towel"),
        default_prompt="Fold the towel twice",
        adapt_to_pi=False,
...
上例的 `default_prompt` 是 `"Fold the towel twice"`，必须与 `inference/task_configs.yaml` 中 `pi05_x1_fold_towel.language_instruction` 完全一致。

### 3.3 计算统计量并训练

```bash
cd ~/X1_openpi/openpi
uv run scripts/compute_norm_stats.py --config-name pi05_x1_fold_towel

XLA_PYTHON_CLIENT_MEM_FRACTION=0.9 \
  uv run scripts/train.py pi05_x1_fold_towel \
  --exp-name=pi05_x1_fold_towel_0717 --overwrite
```

checkpoint 输出路径为 `~/X1_openpi/openpi/checkpoints/pi05_x1_fold_towel/pi05_x1_fold_towel_0717/<STEP>`。
...
ros2 launch multi_realsense multi_cameras.launch.py \
  ...
  serial_left:=<D405_LEFT_WRIST_SERIAL> \
...
ros2 topic hz /cam_wrist_left/cam_wrist_left/color/image_raw
