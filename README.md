...
所有命令默认工程位于 `~/xarm-aio`，ROS 2 发行版为 Humble。以下值需要按现场替换：
...
编辑 `~/xarm-aio/openpi/src/openpi/training/config.py`，在 `_CONFIGS = [` 列表中加入：
...
checkpoint 输出路径为 `~/xarm-aio/openpi/checkpoints/pi05_x1_stack_green_cube/xarm_right_v1/<STEP>`。
...
重新激活 `xarm-aio` 环境，并检查 x_max SDK 激活脚本与扩展路径：
